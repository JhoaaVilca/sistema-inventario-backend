...
public interface KardexRepositorio extends JpaRepository<Kardex, Long> {
    ...
    Page<Kardex> findByProductoIdProductoAndFechaMovimientoBetweenAndTipoMovimientoIgnoreCase(
            Long idProducto, LocalDateTime fechaInicio, LocalDateTime fechaFin, String tipoMovimiento, Pageable pageable);
    ...
    Page<Kardex> findByProductoIdProductoAndTipoMovimientoIgnoreCase(Long idProducto, String tipoMovimiento, Pageable pageable);
    ...
    Page<Kardex> findByFechaMovimientoBetweenAndTipoMovimientoIgnoreCase(
            LocalDateTime fechaInicio, LocalDateTime fechaFin, String tipoMovimiento, Pageable pageable);

    // Obtener todos los movimientos de Kardex (ordenado por Pageable)
    Page<Kardex> findAll(Pageable pageable);

    Page<Kardex> findByTipoMovimientoIgnoreCase(String tipoMovimiento, Pageable pageable);
    ...
    // Último movimiento ANTES de una fecha, para calcular saldo inicial del periodo
    List<Kardex> findTop1ByProductoIdProductoAndFechaMovimientoBeforeOrderByFechaMovimientoDesc(
            Long idProducto, LocalDateTime fechaReferencia);
}
